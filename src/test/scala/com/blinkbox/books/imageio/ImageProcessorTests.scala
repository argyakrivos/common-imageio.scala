...
class ImageProcessorTests extends FunSuite with BeforeAndAfter with ImageChecks {
  import com.blinkbox.books.imageio.ImageProcessorTests._
  ...
  test("Transform png with transparency channel to jpeg (CP-1782)") {
    for (filename <- List("dummy.png", "crash_test_dummy.png")) {
      val transparencyPngData = IOUtils.toByteArray(ImageProcessorTests.getClass.getResourceAsStream("/" + filename))
